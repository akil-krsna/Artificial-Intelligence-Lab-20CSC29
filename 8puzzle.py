...
class Node:
    ...
    # comparators used for arranging nodes based on (g+h) values where g is depth and h is no of misplaced cells
...
#fuction used for checking curr state of matrix is already visited or not
...
#finding shuffled matrix from by swappimg curr space cell with (new_i,new_j)
...
def A_star(source, original, goal_state):
    ...
    visited = [original]
    paths = []
    #min heap used forarranging nodes based on g+h value
    pq = [(0, source)]
    while pq:
        ...
        for k in range(4):
            ...
            #condition used for checking whether new_i and new_j are valid indices or not
